/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     11/17/2016 2:56:43 PM                        */
/*==============================================================*/
...
drop table if exists GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION_STATUS;
...
drop table if exists GENOMIC_REF_DATA.CONFIGURATION_TYPE;

drop table if exists GENOMIC_REF_DATA.CONFIGURATION_VARIABLE_TYPE;

drop table if exists GENOMIC_REF_DATA.CURATOR_DATA;

drop table if exists GENOMIC_REF_DATA.CURATOR_DATA_RELEASE;

drop table if exists GENOMIC_REF_DATA.DESGNTN_RELATIONSHIP_TYPE;

drop table if exists DESIGNATION_TEST_RESULT_GROUP;

drop table if exists DESIGNATION_XX_ALLELE_GROUP;
...
drop table if exists GENOMIC_REF_DATA.GENOMIC_REF_DATA_CONFIGURATION;

drop table if exists GENOMIC_REF_DATA.GENOMIC_REF_OBJECT_STATUS;
...
drop table if exists GENOMIC_REF_DATA.NOMENCLATURE_MISMATCH;
...
drop table if exists GENOMIC_REF_DATA.XX_ALLELE_GROUP_STATUS;
...
(
   ...
   NOMENCLATURE_FORMAT_VER_CDE varchar(10) comment 'A valid value to represent a particular kind or type of Nomenclature Format Version; said codes may be unique and/or vary by Gene Family   For HLA Alleles Values are V2 = V2 format, V3 = V3 format',
   ...
   HLA_EXPRESSION_CHARACTER_CDE varchar(1) comment 'A single character code used as an optional suffix on HLA alleles.  Values are N = null, L = Low Cell Surface, S = Soluable, C = Cytoplasm Present,  A = Aberrant, Q = Questionable',
...
(
   ALLELE_AUD_IID       numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele audit record within the Enterprise.',
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   CHANGE_TS            timestamp not null comment 'The timestamp for the record change.',
   ...
   CHANGE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program changed the record. ',
...
(
   ALLELE_CODE_DESGNTN_AUD_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Code Designation Audit record within the Enterprise.  ',
   ALLELE_CODE_DESIGNATION_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Code Designation within the Enterprise.  ',
   CHANGE_TS            timestamp not null comment 'The timestamp for the record change.',
   ALLELE_CODE_DESIGNATION_NME varchar(50) not null comment 'A textual ''label'' or name (that corresponds to a valid code) by which an Allele Code Designation is known/can be recognized  This name is a concatenation of Gene Family, Gene Locus, Allele Family and Allele Code.  HLA-A*02:AB (generic) or HLA-A*02:AAK (allele specific)',
   ALLELE_CDE           varchar(10) not null comment 'A valid value to represent an Allele Code, typically represented with 4-5 alpha characters.  e.g. AB = ''01/02''.  Letters that can be mistaken for numbers are excluded from being incorproated into said codes (O,Q, I, L)',
   ...
   key AK_ALLELE_CODE_DESIGNATION_AUD (ALLELE_CODE_DESIGNATION_IID, CHANGE_TS)
);
...
(
   ...
   DESGNTN_RELATIONSHIP_TYPE_CDE varchar(10) not null,
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
/*==============================================================*/
/* Table: ALLELE_CODE_DESIGNATION_STATUS                        */
/*==============================================================*/
create table GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION_STATUS
(
   ALLELE_CODE_DESNGTN_STATUS_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Code Designation Status record within the Enterprise.  ',
   ALLELE_CODE_DESIGNATION_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Code Designation within the Enterprise.  ',
   ALLELE_CODE_DESGTN_STATUS_CDE varchar(10) not null comment 'A valid value to represent a specific allele code designation status.  Values are Valid = Valid, Depr = Deprecated, and Invalid = Invalid.  Note - until an allele goes through the validation process it resides in staging structures; if determined to be NOT Valid, the record is removed and NEVER operationalized.  ',
   ALLELE_CODE_DESGTN_STATUS_DTE timestamp not null comment 'The date that the allele code designation status was established/defined.',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (ALLELE_CODE_DESNGTN_STATUS_IID),
   key AK_AK_ALLELE_CODE_DESGNTN_STATUS (ALLELE_CODE_DESIGNATION_IID, ALLELE_CODE_DESGTN_STATUS_CDE, ALLELE_CODE_DESGTN_STATUS_DTE)
);
...
(
   ...
   COMMON_WELL_DOC_REF_URL varchar(256) not null comment 'The URL location for the manifest publishing common well documented alleles',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   COMMON_WELL_DOC_STATUS_CDE varchar(20) not null comment 'A valid value to represent a specific status for a common well documented allele (applicable to HLA gene locus).  Examples are COMMON, WELLDOC, and NOTCWD
            ',
   COMMON_WELL_DOC_VER_NUM varchar(10) not null comment 'A particular version of the decision/manifest that declared an allele to be common/well documented',
   COMMON_WELL_DOC_REF_URL varchar(256) not null comment 'The URL location for the manifest publishing common well documented alleles',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
...
(
   ALLELE_NUCLEOTIDE_SEQ_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Nucleotide Sequence within the Enterprise.  ',
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   ...
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (ALLELE_NUCLEOTIDE_SEQ_IID)
);
...
(
   ALLELE_REJECTION_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents anAllele Rejection record within the Enterprise.  ',
   LONG_FORMAT_NME      varchar(50) not null comment 'A textual ''label'' or name (that corresponds to a valid code) by which an Annotation is known/can be recognized.  For HLA Alleles, this name can vary from full nomenclature consisting of Gene Family, Locus with 2, 3 or 4 distinct fields of detail (HLA-A*01:01, HLA-A*01:01:01, HLA-A*01:01:01:02).',
...
(
   ...
   ALLELE_RELATIONSHIP_TYPE_CDE varchar(10) not null comment 'A valid value to represent a specific kind of relationship that may exists between alleles. Values are RENAME = Renamed Alleles, and SEQREF = Sequence Reference Alleles',
   ALLELE_RELATIONSHIP_REASON_TXT varchar(100) comment 'A textual description for the reason why a particular allele relationship exists.',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   ALLELE_RELATIONSHIP_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a specific relationships between alleles within the Enterprise.  ',
   ALLELE_RELATIONSHIP_REASON_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Relationship Reason record within the Enterprise.  ',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   ALLELE_STATUS_IID    numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents anAllele Status record within the Enterprise.  ',
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   ALLELE_STATUS_CDE    varchar(10) not null comment 'A valid value to represent a specific allele status.  Values are Valid = Valid, Depr = Deprecated, and Invalid = Invalid.  Note - until an allele goes through the validation process it resides in staging structures; if determined to be NOT Valid, the record is removed and NEVER operationalized.',
...
(
   ...
   ALLELE_IN_NOM_FILE_IND varchar(1) not null,
   primary key (ALLELE_IID)
);
...
(
   ...
   BEHAVIOR_CATEGORY_CDE varchar(10) not null comment 'A valid value to represent a specific kind of behavior category that can be applied to alleles.  Values include ATGNBND = Antigen Binding, KIRLGND = Kir Ligand, TCE = Tcell Epitope',
   BEHAVIOR_GROUP_CONTEXT_CDE varchar(10) comment 'A valid value to represent a specific kind of context that can be applied to allele behavior groups. Preliminary values are ARS = ARS Sets, EXP = EXP Sets, C1 = [need definition]) but need further review',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   BEHAVIOR_GROUP_CONTEXT_CDE varchar(10) not null comment 'A valid value to represent a specific kind of context that can be applied to allele behavior groups. Preliminary values are ARS = ARS Sets, EXP = EXP Sets, C1 = [need definition]) but need further review',
...
(
   BHVR_ALLELE_GRP_ALLELE_AUD_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a Behavior Allele Group Allele Audit record within the Enterprise.  ',
   BEHAVIOR_ALLELE_GROUP_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a behavior allele group within the Enterprise.  ',
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   CHANGE_TS            timestamp not null comment 'The timestamp for the record change.',
   CHANGE_TYPE_CDE      varchar(10) not null comment 'The type of change made to the record.  U= Update (001), I = Insert (002) , D = Delete (003).  Legacy values are in parenthesis',
...
(
   COMMON_WELL_DOC_STATUS_CDE varchar(10) not null comment 'A valid value to represent a specific status for a common well documented allele (applicable to HLA gene locus).  Examples are COMMON, WELLDOC, and NOTCWD
            ',
   COMMON_WELL_DOC_STATUS_NME varchar(50) not null comment 'A textual ''label'' or name (that corresponds to a valid code) by which  a Common Well Documented Status is known/can be recognized',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   CONFIG_VARIABLE_TYPE_CDE varchar(10) not null comment 'A valid value to represent a particular kind or type of variable being managed for Genomic Reference Data.  For example GENELOCUS is a CDE with a corresponding name of Gene Locus and used as kind of variable with configuration type NOMFILE_EXCL.  ',
   CONFIG_VARIABLE_TYPE_NME varchar(50) not null comment 'A name or label used to add clarity to a corresponding variable CDE',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (CONFIG_VARIABLE_TYPE_CDE)
);

alter table GENOMIC_REF_DATA.CONFIGURATION_VARIABLE_TYPE comment 'A classification or taxonomy used to define specific variabl';
...
(
   CURATOR_DATA_IID     numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Curator Data record within the Enterprise.  ',
   GENE_FAMILY_CDE      varchar(10) comment 'A valid value to represent a specific kind of gene system/family (e.g. HLA = Human Leukocyte Antigen, KIR = Killer Immunoglobin Receptor, CCR = C-C Chemokine Receptor, ABO = ABO Blood Group',
...
(
   CURATOR_DATA_RELEASE_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Curator Data Release record within the Enterprise.  ',
   CURATOR_DATA_IID     numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Curator Data record within the Enterprise.  ',
...
(
   DESGNTN_RELATIONSHIP_TYPE_CDE varchar(10) not null,
   DESGNTN_RELATIONSHIP_TYPE_NME varchar(50) not null,
   DESGNTN_RELATIONSHIP_TYPE_TXT varchar(100),
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (DESGNTN_RELATIONSHIP_TYPE_CDE),
   key AK_AK_DESGNTN_RELATIONSHIP_TYPE (DESGNTN_RELATIONSHIP_TYPE_NME)
);

/*==============================================================*/
/* Table: DESIGNATION_TEST_RESULT_GROUP                         */
/*==============================================================*/
create table DESIGNATION_TEST_RESULT_GROUP
(
   ALLELE_CODE_DESIGNATION_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Code Designation within the Enterprise.  ',
   ...
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (ALLELE_CODE_DESIGNATION_IID, TEST_RESULT_ALLELE_GRP_IID)
);

/*==============================================================*/
/* Table: DESIGNATION_XX_ALLELE_GROUP                           */
/*==============================================================*/
create table DESIGNATION_XX_ALLELE_GROUP
(
   ALLELE_CODE_DESIGNATION_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an Allele Code Designation within the Enterprise.  ',
   ...
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (ALLELE_CODE_DESIGNATION_IID, XX_ALLELE_GROUP_IID)
);
...
(
   GENE_FAMILY_CDE      varchar(10) not null comment 'A valid value to represent a specific kind of gene system/family (e.g. HLA = Human Leukocyte Antigen, KIR = Killer Immunoglobin Receptor, CCR = C-C Chemokine Receptor, ABO = ABO Blood Group',
   GENE_FAMILY_NME      varchar(50) not null comment 'The name for a given gene family.  For example for HLA the description is Human Leukocyte Antigen',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   GENOME_DATABASE_VER_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a genome database version within the Enterprise.  ',
   GENE_FAMILY_CDE      varchar(10) not null comment 'A valid value to represent a specific kind of gene system/family (e.g. HLA = Human Leukocyte Antigen, KIR = Killer Immunoglobin Receptor, CCR = C-C Chemokine Receptor, ABO = ABO Blood Group',
   DATABASE_VERSION_TXT varchar(50) not null comment 'The description given to a published database. the name has historically followed the following pattern: <major release number>.<minor release number>.<patch/correction> the name is established by committee',
...
(
   ...
   GENOMIC_REF_OBJECT_TYPE_CDE varchar(10) not null comment 'A valid value to represent a particular kind or type of Genomic Reference Object; valid types may vary by gene family.  Values are ALLELE = Allele, MAC = Allele Code Designation, SER = Serology, XXGRP = XX Allele Group, and TSTRSLT = Test Result Allele Group',
...
(
   GENOMIC_REF_OBJECT_TYPE_CDE varchar(10) not null comment 'A valid value to represent a particular kind or type of Genomic Reference Object; valid types may vary by gene family.  Values are ALLELE = Allele, MAC = Allele Code Designation, SER = Serology, XXGRP = XX Allele Group, and TSTRSLT = Test Result Allele Group',
   GENOMIC_REF_OBJECT_TYPE_NME varchar(50) not null comment 'A textual ''label'' or name (that corresponds to a valid code) by which a Genomic Reference Object is known/can be recognized.  For Example, Allele, Allele Code Designation, etc.',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   GENOMIC_REF_DATA_CONFIG_IID numeric(9,0) not null,
   CONFIGURATION_TYPE_CDE varchar(20) comment 'A valid value to represent a particular high level categorization of a configuration to be managed for Genomic Reference Data.  For example NOMFILE_EXCL represents a Nomenclature File Exclusions within which it may be possible to have more than one kindn of variable type defined.  Current the case is only one type is applicable - GENELOCUS and multiple instances of configurations (i.e. more than one gene locus) can be defined within this broader category of NOMFILE_EXCL.',
   CONFIG_VARIABLE_TYPE_CDE varchar(10) comment 'A valid value to represent a particular kind or type of variable being managed for Genomic Reference Data.  For example GENELOCUS is a CDE with a corresponding name of Gene Locus and used as kind of variable with configuration type NOMFILE_EXCL.  ',
   CONFIGURATION_VARIABLE_IID numeric(9,0) comment 'An instance value of a variable type that is stored as an IID',
   CONFIGURATION_VARIABLE_CDE varchar(10) comment 'An instance value of a variable type that is stored as an CDE',
   CONFIGURATION_VARIABLE_NME varchar(50) comment 'An instance value of a variable type that is stored as an NME',
   CONFIGURATION_VARIABLE_TXT varchar(2000) comment 'An instance value of a variable type that is stored as an TXT',
   CONFIGURATION_VARIABLE_IND varchar(1) comment 'An instance value of a variable type that is stored as an IND',
   CONFIGURATION_VARIABLE_DTE timestamp comment 'An instance value of a variable type that is stored as a DTE',
   CONFIGURATION_VARIABLE_TS timestamp comment 'An instance value of a variable type that is stored as an TS',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (GENOMIC_REF_DATA_CONFIG_IID)
);
...
(
   GENOMIC_REF_OBJECT_STATUS_CDE varchar(10) not null comment 'A valid value to represent a specific status for an Allele, Allele Code Designation or XX Allele Group.  Values are Valid = Valid, Depr = Deprecated, and Invalid = Invalid.  Note - until an allele goes through the validation process it resides in staging structures; if determined to be NOT Valid, the record is removed and NEVER operationalized.  ',
   GENOMIC_REF_OBJECT_STATUS_NME varchar(50) not null comment 'A textual ''label'' or name (that corresponds to a valid code) by which an Allele Status is known/can be recognized',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (GENOMIC_REF_OBJECT_STATUS_CDE),
   key AK_ALLELE_STATUS_DEF (GENOMIC_REF_OBJECT_STATUS_NME)
);

alter table GENOMIC_REF_DATA.GENOMIC_REF_OBJECT_STATUS comment 'The valid values for statuses that can be associated/linked ';
...
(
   GENOMIC_REFERENCE_OBJECT_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a Genomic Reference Object. ',
   ANTIGEN_EID          numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a legacy (sybase) Antigen record within the Enterprise.  ',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (GENOMIC_REFERENCE_OBJECT_IID, ANTIGEN_EID)
);
...
(
   HLA_EXPRESSION_CHARACTER_CDE varchar(1) not null comment 'A single character code used as an optional suffix on HLA alleles.  Values are N = null, L = Low Cell Surface, S = Soluable, C = Cytoplasm Present,  A = Aberrant, Q = Questionable',
...
(
   NOMENCLATURE_FORMAT_VER_CDE varchar(10) not null comment 'A valid value to represent a particular kind or type of Nomenclature Format Version; said codes may be unique and/or vary by Gene Family   For HLA Alleles Values are V2 = V2 format, V3 = V3 format',
...
(
   NOMENCLATURE_MISMATCH_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele nomenclature mismatch record within the Enterprise.  ',
   GENE_FAMILY_CDE      varchar(10) not null comment 'A valid value to represent a specific kind of gene system/family (e.g. HLA = Human Leukocyte Antigen, KIR = Killer Immunoglobin Receptor, CCR = C-C Chemokine Receptor, ABO = ABO Blood Group',
   ALLELE_NME           varchar(100) not null comment '''An allele name that is mismatch between nomenclature file and both staging and ORD.  Possible values K*01:01:01:02, TAP1*02:01:02, A*01:01:69
            ',
   ALLELE_PUBLISHED_DTE timestamp not null comment 'The date that the allele tied to the accession number was published or discovered.',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (NOMENCLATURE_MISMATCH_IID)
);
...
(
   ...
   PUBL_SAMPLE_ETHNICITY_NME varchar(50) not null comment 'A label or short description of the ethnicity connected to a published sample.  Note - ethnicities not currently aligned to those ethnicities used at NMDP',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   ...
   RNA_AMINO_ACID_TRANSLATION_TXT text comment 'The nucleotide sequence as translanted to RNA amino acids.',
   RNA_AMINO_ACID_TRANSLATION_ID numeric(9,2) comment 'The identifier that is linked to the translation sequence published by IMGT',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   PUBL_ALLELE_GROUP_VER_AUD_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a Published Allele Group Version Audit record within the Enterprise.  ',
   PUBLISHED_ALLELE_GROUP_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a published allele group (as curated) within the Enterprise.  ',
   GENOME_DATABASE_VER_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a genome database version within the Enterprise.  ',
   CHANGE_TS            timestamp not null comment 'The timestamp for the record change.',
   ALLELE_LIST_IID      numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele list within the Enterprise.  ',
...
(
   SRCH_DTRM_RELT_TYPE_CDE varchar(10) not null comment 'A valid value to represent a specific kind of relationshp between ''pairs'' of search determinants. Values are P = Parent and C = Child).  This value provides context to the ''related'' search determinant referred to as #2.',
...
(
   ...
   GENE_LOCUS_IID       numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a gene locus within the Enterprise.  ',
   PARENT_SEROLOGY_IID  numeric(9,0) comment 'A system generated identifier (surrogate key) that uniquely represents a serology within the Enterprise. ',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   SEROLOGY_IID         numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a serology within the Enterprise. ',
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   SEROLOGY_CONTEXT_CDE varchar(20) not null comment 'A valid value to represent a particular context used when linking Serology to Alleles.  Values include UMAMBIG = Unambiguous, POSSIBLE = Possible, ASSUME = Assumed, and  EXPERT = Expoert Assigned',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   SEROLOGY_CONTEXT_CDE varchar(20) not null comment 'A valid value to represent a particular context used when linking Serology to Alleles.  Values include UMAMBIG = Unambiguous, POSSIBLE = Possible, ASSUME = Assumed, and  EXPERT = Expoert Assigned',
   SEROLOGY_CONTEXT_NME varchar(50) not null comment 'A textual ''label'' or name (that corresponds to a valid code) by which a Serology Context is known/can be recognized.  ',
   SEROLOGY_CONTEXT_TXT varchar(100) comment 'A textual description that provides additional context or explanation for the serology context and how it should be interpreted',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   ...
   GENE_LOCUS_IID       numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a gene locus within the Enterprise.  ',
   TEST_RESLT_REPRT_TYPE_CDE varchar(10) not null comment 'A valid value to represent a Test Result Reporting Type.   NTST = Not Tested, TPOS = Tested and Present, TNEG = Tested and NOT Present. NEW = New Allele',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   PREFERRED_PUBLICATION_IND varchar(1) not null comment 'Indicates if the Test Result Allele Group is preferred to be published (versus the corresponding Allele Code Designation(s) to which it is mapped) .  Values are Y and N   This ONLY impacts internal consumers (i.e. NMDP ONLY) ',
...
(
   ...
   TEST_RESLT_REPRT_TYPE_TXT varchar(100) comment 'A label or short textual description that represents a test level.  Example of what would be represented include Not Tested, Tested and Present, Tested and NOT Present',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
...
(
   ...
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   PREFERRED_PUBLICATION_IND varchar(1) not null comment 'Indicates if the XX Allele Group is preferred to be published (versus the corresponding Allele Code Designation(s) to which it is mapped) .  Values are Y and N   This would impact what is provided to external consumer (i.e. outside of NMDP)',
...
(
   XX_ALLELE_GRP_ALLELE_AUD_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents a XX Group Allele Audit record within the Enterprise.  ',
   XX_ALLELE_GROUP_IID  numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an XX Group within the Enterprise. ',
   ALLELE_IID           numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an allele within the Enterprise.  ',
   CHANGE_TS            timestamp not null comment 'The timestamp for the record change.',
   CHANGE_TYPE_CDE      varchar(10) not null comment 'The type of change made to the record.  U= Update (001), I = Insert (002) , D = Delete (003).  Legacy values are in parenthesis',
...
/*==============================================================*/
/* Table: XX_ALLELE_GROUP_STATUS                                */
/*==============================================================*/
create table GENOMIC_REF_DATA.XX_ALLELE_GROUP_STATUS
(
   XX_ALLELE_GROUP_STATUS_IID numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an  XX Allele Group record within the Enterprise.  ',
   XX_ALLELE_GROUP_IID  numeric(9,0) not null comment 'A system generated identifier (surrogate key) that uniquely represents an XX Group within the Enterprise. ',
   XX_ALLELE_GROUP_STATUS_CDE varchar(10) not null comment 'A valid value to represent a specific xx allele group status.  Values are Valid = Valid, Depr = Deprecated, and Invalid = Invalid.  Note - until an allele goes through the validation process it resides in staging structures; if determined to be NOT Valid, the record is removed and NEVER operationalized.  ',
   XX_ALLELE_GROUP_STATUS_DTE timestamp not null comment 'The date that the allele code designation status was established/defined.',
   CREATE_TS            timestamp not null comment 'The record created timestamp in the database',
   CREATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program created the record',
   UPDATE_TS            timestamp not null comment 'The record updated timestamp in the database',
   UPDATE_BY_ID         varchar(50) not null comment 'The user id / unique id of the person / program updated the record. ',
   primary key (XX_ALLELE_GROUP_STATUS_IID),
   key AK_AK_XX_ALLELE_GROUP_STATUS (XX_ALLELE_GROUP_IID, XX_ALLELE_GROUP_STATUS_CDE, XX_ALLELE_GROUP_STATUS_DTE)
);
...
alter table GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION_RELT add constraint FK_ALLELE_CODE_DESGNTN_RELT_03 foreign key (DESGNTN_RELATIONSHIP_TYPE_CDE)
      references GENOMIC_REF_DATA.DESGNTN_RELATIONSHIP_TYPE (DESGNTN_RELATIONSHIP_TYPE_CDE) on delete restrict on update restrict;

alter table GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION_STATUS add constraint FK_ALLELE_CODE_DESGNTN_STAT_01 foreign key (ALLELE_CODE_DESIGNATION_IID)
      references GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION (ALLELE_CODE_DESIGNATION_IID) on delete restrict on update restrict;

alter table GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION_STATUS add constraint FK_ALLELE_CODE_DESGNTN_STAT_02 foreign key (ALLELE_CODE_DESGTN_STATUS_CDE)
      references GENOMIC_REF_DATA.GENOMIC_REF_OBJECT_STATUS (GENOMIC_REF_OBJECT_STATUS_CDE) on delete restrict on update restrict;
...
alter table DESIGNATION_TEST_RESULT_GROUP add constraint FK_DESGNTN_TEST_RESULT_GRP_01 foreign key (TEST_RESULT_ALLELE_GRP_IID)
      references GENOMIC_REF_DATA.TEST_RESULT_ALLELE_GROUP (TEST_RESULT_ALLELE_GRP_IID) on delete restrict on update restrict;

alter table DESIGNATION_TEST_RESULT_GROUP add constraint FK_DESGNTN_TEST_RESULT_GRP_02 foreign key (ALLELE_CODE_DESIGNATION_IID)
      references GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION (ALLELE_CODE_DESIGNATION_IID) on delete restrict on update restrict;

alter table DESIGNATION_XX_ALLELE_GROUP add constraint FK_DESGNTN_XX_ALLELE_GROUP_01 foreign key (ALLELE_CODE_DESIGNATION_IID)
      references GENOMIC_REF_DATA.ALLELE_CODE_DESIGNATION (ALLELE_CODE_DESIGNATION_IID) on delete restrict on update restrict;

alter table DESIGNATION_XX_ALLELE_GROUP add constraint FK_DESGNTN_XX_ALLELE_GROUP_02 foreign key (XX_ALLELE_GROUP_IID)
...
alter table GENOMIC_REF_DATA.GENOMIC_REF_DATA_CONFIGURATION add constraint FK_GENOMIC_REF_DATA_CONFIG_01 foreign key (CONFIGURATION_TYPE_CDE)
      references GENOMIC_REF_DATA.CONFIGURATION_TYPE (CONFIGURATION_TYPE_CDE) on delete restrict on update restrict;

alter table GENOMIC_REF_DATA.GENOMIC_REF_DATA_CONFIGURATION add constraint FK_GENOMIC_REF_DATA_CONFIG_02 foreign key (CONFIG_VARIABLE_TYPE_CDE)
      references GENOMIC_REF_DATA.CONFIGURATION_VARIABLE_TYPE (CONFIG_VARIABLE_TYPE_CDE) on delete restrict on update restrict;
...
alter table GENOMIC_REF_DATA.SEROLOGY add constraint FK_FK_SEROLOGY_01 foreign key (PARENT_SEROLOGY_IID)
...
alter table GENOMIC_REF_DATA.XX_ALLELE_GROUP_STATUS add constraint FK_XX_ALLELE_GROUP_STATUS_01 foreign key (XX_ALLELE_GROUP_IID)
      references GENOMIC_REF_DATA.XX_ALLELE_GROUP (XX_ALLELE_GROUP_IID) on delete restrict on update restrict;

alter table GENOMIC_REF_DATA.XX_ALLELE_GROUP_STATUS add constraint FK_XX_ALLELE_GROUP_STATUS_02 foreign key (XX_ALLELE_GROUP_STATUS_CDE)
      references GENOMIC_REF_DATA.GENOMIC_REF_OBJECT_STATUS (GENOMIC_REF_OBJECT_STATUS_CDE) on delete restrict on update restrict;
