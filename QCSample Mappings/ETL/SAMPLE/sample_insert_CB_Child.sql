...
ml.MASTER_LOT_ID || '.' || lpad(ROW_NUMBER() OVER (PARTITION BY qcdid.QC_DID ORDER BY qcdid.NMDP_DID) + e.VIALS_LEFT,6,'0') || 'K' as SAMPLE_ID,
ml.MASTER_LOT_IID,
...
inner join ETL_STAGE.QCSAMPLE_INV_SYBASE_QC_DID qcdid on trim(e.QC_MASTER_LOT_ID) = trim(qcdid.QC_DID)
where e.LOCAL_CBU_ID is not null and e.FP_QUANTITY_LEFT > 0;
