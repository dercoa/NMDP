...
e.DATE_CREATE as SAMPLE_CREATION_DTE,
...
--inner join ETL_STAGE.QCSAMPLE_INV_SYBASE_QC_DID qcdid on trim(replace(ss.SAMPLE_SOURCE_ID,'QC_DRIVE','QC')) = trim(qcdid.QC_DID)
inner join ETL_STAGE.QCSAMPLE_INV_SYBASE_QC_DID qcdid on cast(replace(ss.SAMPLE_SOURCE_ID,'QC_DRIVE','') as number) = cast(replace(qcdid.QC_DID,'QC', '') as number)
where e.TOTAL_FILTER_PAPER is not null;
